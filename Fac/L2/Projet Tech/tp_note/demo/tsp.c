...
/**
 * @brief Check a distance matrix is valid.
 * @param size problem size
 * @param distmat distance matrix
 * @return true if valid (symmetric and zero diagonal)
 */

bool distmat_check(uint size, uint *distmat) {
  assert(size >= 2);
  assert(distmat);
  for (uint i = 0; i < size; i++) {
    // symmetric
    for (uint j = 0; j < i; j++)
      if (distmat[i * size + j] != distmat[j * size + i]) return false;
    // zero diagonal
    if (distmat[i * size + i] != 0) return false;
  }
  return true;
}
