...
node createNode(position pos, double cost, node parent, heuristic h, grid G) {
  node n = malloc(sizeof(*n));
  n->pos = pos;
  n->cost = cost;
  n->score = n->cost + h(n->pos, G.end, &G);
  n->parent = parent;

  return n;
}

int fcmpNode(const void* x, const void* y) {
  const node n1 = *(node*)x;
  const node n2 = *(node*)y;

  return (n1->score < n2->score) ? -1 : (n1->score > n2->score);
}

double A_star(grid G, heuristic h) {
  heap Q = heap_create(8 * G.X * G.Y, fcmpNode);
  node s = createNode(G.start, 0, NULL, h, G);
  heap_add(Q, s);
  ...
  while (!heap_empty(Q)) {
    ...
    if (u->pos.x == G.end.x && u->pos.y == G.end.y) {
      ...
      while (par != NULL) {
        G.mark[par->pos.x][par->pos.y] = MK_PATH;
        par = par->parent;
      }
    }
    ...
    for (int i = -1; i <= 1; i++) {
      for (int j = -1; j <= 1; j++) {
        ...
        if (G.mark[vp.x][vp.y] == MK_USED)
          continue;

        double c = u->cost + weight[G.texture[vp.x][vp.y]];
