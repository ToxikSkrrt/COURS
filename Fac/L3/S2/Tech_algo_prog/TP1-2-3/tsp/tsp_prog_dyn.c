...
int ExtractPath(cell** D, int t, int S, int n, int* Q)
{
  ...
  */
  ...
  while (Q[k - 1] != n - 1)
  { // on s'arrête lorsque le point V[n-1] est atteint
    Q[k] = D[Q[k - 1]][S].pred;
    S = DeleteSet(S, Q[k - 1]);
    k++;
  }
...
double tsp_prog_dyn(point* V, int n, int* Q)
{
  ...
  */
  ...
  for (int S = 0; S < C && running; S++) {
    for (int t = 0; t < L && running; t++) {
      int T = DeleteSet(S, t);
      ...
      if (!T) {
        ...
        double min = DBL_MAX;
        int pred;
        for (int x = 0; x < L && running; x++) {
          if (x && T) {
            double res = D[x][T].length + dist(V[x], V[t]);
            if (res < min) {
              min = res;
              pred = x;
            }
