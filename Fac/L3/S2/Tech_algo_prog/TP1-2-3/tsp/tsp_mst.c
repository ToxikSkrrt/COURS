...
double tsp_mst(point* V, int n, int* P, graph T) {
  ...
  edge* E = malloc((n * (n - 1) / 2) * sizeof(edge)); // à remplacer par un malloc(...)

  int cpt = 0;
  for (int u = 0; u < n && running; u++) {
    for (int v = u + 1; v < n && running; v++) {
      E[cpt].u = u;
      E[cpt].v = v;
      E[cpt].weight = dist(V[u], V[v]);
      cpt++;
    }
  }
  ...
  for (int i = 0, j = 0; (i < n * (n - 1) / 2) && (j < n - 1) && running; i++) {
    int u = E[i].u, v = E[i].v;
    int Fu = Find(u, parent), Fv = Find(v, parent);
    if (Fu != Fv) {
      addEdge(T, u, v);
      Union(Fu, Fv, parent, rank);
      j++;
      // SDL_Delay(100);
      // drawGraph(V, n, P, T);
    }
