...
let () = Format.printf "compo : %d@," ((compo (fun x -> 2*x) (fun x -> x+1)) 4)
...
let apply_movement move pos =
  let x, y = pos in
  let dir, speed = move in
  if speed <= 0 then
    pos
  else
    match dir with
...
let apply_movement_list (move : movement) list =
  List.map (apply_movement move) list

let find_farthest_point list =
  List.fold_left (fun x y -> let a, b = x in let sum_x = abs a + abs b in let c, d = y in let sum_y = abs c + abs d in if sum_x > sum_y then x else y) (0, 0) list

let x, y = find_farthest_point list_pos
let () = Format.printf "FFP : (%d, %d)@," x y
