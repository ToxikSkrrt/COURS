...
let infix_string_of_term term =
  let rec aux term =
    ...
    | Div (x, y) -> "(" ^ aux x ^ " / " ^ aux y ^ ")"
    | Mod (x, y) -> "(" ^ aux x ^ " % " ^ aux y ^ ")"
    | Min (x, y) -> "(" ^ aux x ^ " A " ^ aux y ^ ")"
    | Max (x, y) -> "(" ^ aux x ^ " v " ^ aux y ^ ")"
  in aux term

let _ = infix_string_of_term (Add(Int(4),Mul(Int(6),Int(5))))
...
let polish_string_of_term term =
  let rec aux term =
    ...
    | Div (x, y) -> "/ " ^ aux x ^ aux y
    | Mod (x, y) -> "% " ^ aux x ^ aux y
    | Min (x, y) -> "A " ^ aux x ^ aux y
    | Max (x, y) -> "v " ^ aux x ^ aux y
  in aux term

let _ = polish_string_of_term (Add(Int(4),Mul(Int(6),Int(5))))
